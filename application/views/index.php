        ...
        <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar" style="background-color:teal;">
            ...
            <a class="sidebar-brand d-flex align-items-center justify-content-center" href="#">
                <div class="sidebar-brand-icon">
                    <!-- <img src="<?=base_url('/uploads/');?><?=_app('logo');?>" alt="Logo" height="50" width="70"> -->
                </div>
                <div class="sidebar-brand-text mx-3">INV-KOPERASI<sup></sup></div>
            </a>
            ...
            <?php if($this->session->userdata('level')=='super_user' || $this->session->userdata('level')=='admin'): ?>
            <!-- Nav Item - Data Pegawai -->
            <li class="nav-item <?=isset($mMaster)?'active':'';?>">
                <a class="nav-link <?=isset($mMaster)?'':'collapsed';?>" href="#" data-toggle="collapse"
                    data-target="#collapsePages" aria-expanded="true" aria-controls="collapsePages">
                    <i class="fas fa-fw fa-folder"></i>
                    <span>Data Master</span>
                </a>
                <div id="collapsePages" class="collapse <?=isset($mMaster)?'show':'';?>" aria-labelledby="headingPages"
                    data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        <?php if($this->session->userdata('level')=='super_user'): ?>
                        <a class="collapse-item <?=isset($mProfil)?'active':'';?>"
                            href="<?=base_url('menu/profil');?>">Profil</a>
                        <a class="collapse-item <?=isset($mSatuan)?'active':'';?>"
                            href="<?=base_url('menu/satuan');?>">Satuan</a>
                        <a class="collapse-item <?=isset($mTujuan)?'active':'';?>"
                            href="<?=base_url('menu/tujuan');?>">Tujuan</a>
                        <?php endif; ?>
                        <a class="collapse-item <?=isset($mBarang)?'active':'';?>"
                            href="<?=base_url('menu/barang');?>">Barang</a>
                        <a class="collapse-item <?=isset($mCustomer)?'active':'';?>"
                            href="<?=base_url('menu/customer');?>">Customer</a>
                    </div>
                </div>
            </li>
            <!-- Nav Item - Create Invoice -->
            <li class="nav-item <?=isset($mCreateInv)?'active':'';?>">
                <a class="nav-link" href="<?=base_url('menu/create_inv');?>">
                    <i class="fas fa-fw fa-chart-line"></i>
                    <span>Create Invoice</span></a>
            </li>
            <!-- Nav Item - Invoice -->
            <li class="nav-item <?=isset($mInvoice)?'active':'';?>">
                <a class="nav-link" href="<?=base_url('menu/invoice');?>">
                    <i class="fas fa-fw fa-chart-line"></i>
                    <span>Invoice</span></a>
            </li>
            <?php endif; ?>
            ...
            <footer class="sticky-footer bg-white">
                <div class="container my-auto">
                    <div class="copyright text-center my-auto">
                        <span>Copyright &copy; <?= _app('name'); ?> <?= date('Y'); ?></span>
                    </div>
                </div>
            </footer>
            <!-- End of Footer -->
